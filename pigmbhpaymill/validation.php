...
class PigmbhpaymillValidationModuleFrontController implements Services_Paymill_LoggingInterface
{
    ...
    public function initContent($cart, $user, $shopname)
    {
        ...
        if ($result === true) {
            ...
            Tools::redirect('index.php?controller=order-confirmation?key=' . $user->secure_key . '&id_cart=' . (int) $cart->id . '&id_module=' . (int) $paymill->id . '&id_order=' . (int) $paymill->currentOrder);
        } else {
